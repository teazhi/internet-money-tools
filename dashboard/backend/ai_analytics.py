...
class AIAnalytics:
    ...
    def predict_restocking_from_analytics(self, sales_data: Dict[str, int], stock_info: Dict[str, dict], lead_time_days: int = 90) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            prompt = f"""
            You are an inventory management expert for Amazon FBA sellers. Analyze this sales data and provide practical restocking recommendations.
            
            Sales & Stock Analysis (Top 20 products by urgency):
            {json.dumps(velocity_data[:20], indent=2)}
            
            Lead Time: {lead_time_days} days
            
            RULES:
            1. Only recommend products that will run out within the next {lead_time_days + 30} days
            2. Calculate recommended quantity as: (daily_velocity × (lead_time + 30)) × 1.2 (safety buffer)
            3. Set urgency based on days until stockout:
               - critical: < 14 days
               - high: 14-30 days  
               - medium: 30-60 days
               - low: 60+ days
            4. Focus on actionable recommendations only
            
            For each product that needs restocking, provide:
            - asin: the ASIN
            - product_name: use the ASIN (product names not available)
            - recommended_order_quantity: calculated quantity (round to reasonable numbers)
            - urgency: critical/high/medium/low based on rules above
            - reasoning: brief explanation (max 1 sentence)
            - estimated_runout_days: days until stockout at current velocity
            
            Only include products that actually need restocking. Skip products with sufficient stock.
